alert("O formato da data está como Mês/Dia/Ano por causa do formato do input!");
...
function birthDateValidation(date) {
  ...
  const yearLimit = formatedDate[0] <= year.getFullYear() && formatedDate[0] >= 1800;
  const monthLimit = formatedDate[1] <= 12 && formatedDate[1] > 0;
  const dayLimit = formatedDate[2] <= 31 && formatedDate[2] > 0;
...
// Stores last person the function got 
...
function getStoredPeoplesAndShow() {
  ...
  let storedPeoples = JSON.parse(localStorage.getItem("peoples"))
  // console.log("Show peoples",storedPeoples);
  const tbody = document.querySelector('#table tbody');
  tbody.innerHTML = "";
  ...
  if (storedPeoples) {
    ...
    storedPeoples.forEach((person => {
      ...
      let birthDateFormated = person.birthDate.split("-");
      birthDateFormated.reverse()

      tr.id = person.id;
      td_name.innerText = person.name;
